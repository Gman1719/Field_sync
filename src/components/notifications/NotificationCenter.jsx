...
export default function NotificationCenter({ user, setActiveTab }) {
  ...
  const renderPriorityBadge = (priority) => {
    switch (priority) {
      case 'URGENT':
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold uppercase tracking-wider bg-rose-100 text-rose-700 border border-rose-200 animate-pulse">
      ...
      default:
        return (
          <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-medium tracking-wider bg-slate-100 text-slate-600 border border-slate-200">
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-200">
      ...
      <div className="bg-white rounded-xl shadow-xs border border-slate-200 p-4 space-y-4">
        <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-4">
          ...
          <div className="relative flex-1 max-w-md">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2 text-xs bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#1E3A8A]/30 focus:border-[#1E3A8A] transition-all"
            />
          </div>
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 pt-2 border-t border-slate-100 text-xs">
          ...
          <select
            ...
            className="px-3 py-1.5 bg-slate-50 border border-slate-200 rounded-lg text-xs text-slate-700 font-medium focus:outline-none focus:border-[#1E3A8A]"
          >
          ...
          {(priorityFilter !== 'ALL' || categoryFilter !== 'ALL' || searchQuery.trim()) && (
            <button
              ...
              className="text-xs text-blue-600 hover:text-blue-800 font-semibold hover:underline ml-2"
            >
              Reset Filters
            </button>
          )}
          ...
          <div className="ml-auto text-xs text-slate-400">
            Showing <span className="font-semibold text-slate-700">{notifications.length}</span> of{' '}
      ...
      <div className="bg-white rounded-xl shadow-xs border border-slate-200 overflow-hidden divide-y divide-slate-100">
        {loading ? (
          <div className="py-20 text-center space-y-3">
            <RefreshCw className="w-8 h-8 text-[#1E3A8A] animate-spin mx-auto" />
              ...
              >
                ...
                <div className="flex-1 min-w-0 space-y-1">
                  <div className="flex flex-wrap items-center gap-2">
                    <span className={`text-sm font-semibold tracking-tight ${isUnread ? 'text-slate-900 font-bold' : 'text-slate-800'}`}>
                      {n.title}
                    </span>
                    {renderPriorityBadge(n.priority)}
                    {n.type && (
                      <span className="px-2 py-0.5 rounded-md text-[10px] font-mono bg-slate-100 text-slate-500 border border-slate-200/60">
                        {n.type}
                      </span>
                    )}
                    {isUnread && (
                  ...
                  <div className="flex items-center gap-4 pt-1 text-[11px] text-slate-400">
                    ...
                    {n.actionUrl && (
                      <span className="inline-flex items-center gap-1 text-[#1E3A8A] font-semibold group-hover:underline">
      ...
      {pagination.totalPages > 1 && (
        <div className="flex items-center justify-between bg-white px-4 py-3 rounded-xl border border-slate-200 text-xs shadow-xs">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-2 border border-slate-200 rounded-lg hover:bg-slate-50 disabled:opacity-40 disabled:pointer-events-none transition-colors"
              title="Previous Page"
            >

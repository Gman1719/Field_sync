  ...
  if (loading) {
    return (
      <div className="py-20 flex flex-col items-center justify-center text-slate-400">
        ...
        <p className="text-sm font-medium text-slate-600">Loading officer verification scores...</p>
      </div>
    );
  }
          ...
          <div className="p-3 bg-white rounded-xl border border-slate-200 shadow-sm flex items-center gap-3">
            <div className="text-right">
              ...
              <div className={`text-xl font-bold ${
                summary.avgScore >= 80 ? 'text-emerald-600' :
                summary.avgScore >= 60 ? 'text-amber-600' : 'text-rose-600'
              }`}>
                {summary.avgScore}%
              </div>
            </div>
      ...
      <div className="space-y-3">
        {filteredOfficers.length === 0 ? (
          <Card className="py-16 text-center text-slate-400">
            ...
            <p className="text-base font-semibold text-slate-700">No officers match verification filter</p>
            ...
            return (
              <Card
                ...
                className={`transition-all overflow-hidden ${
                  isExpanded ? 'ring-2 ring-[#1E3A8A]/30 border-[#1E3A8A]' : 'hover:border-slate-300'
                }`}
              >
                {/* Header Row */}
                <div
                ...
                >
                  <div className="flex items-center gap-3">
                    <div className="w-10 h-10 rounded-full bg-[#1E3A8A]/10 text-[#1E3A8A] flex items-center justify-center font-bold text-sm">
                  ...
                  <div className="flex items-center gap-4">
                    <div className="text-right">
                      ...
                      <div className={`text-lg sm:text-xl font-bold font-mono ${
                        officer.hasHistory
                          ? officer.trustScore >= 80 ? 'text-emerald-600' :
                            officer.trustScore >= 60 ? 'text-amber-600' : 'text-rose-600'
                          : 'text-slate-400'
                      }`}>
                  ...
                  <div className="p-4 pt-0 border-t border-slate-100 bg-slate-50/50 space-y-4">
                    {!officer.hasHistory ? (
                      <div className="py-8 text-center text-slate-400">
                        ...
                        <p className="text-sm font-medium text-slate-600">No synced verification history for this officer yet</p>
                        <p className="text-xs text-slate-400 mt-1">Audit challenge responses will appear here after synchronization</p>
                      </div>
                    ) : (
                      <>
                        {/* Metrics Pills Grid */}
                        <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-8 gap-2 pt-3">
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            ...
                            <div className="text-base font-bold text-slate-900">{officer.historyCount}</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-emerald-600">Passed</div>
                            <div className="text-base font-bold text-emerald-700">{officer.passed}</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-rose-600">Failed</div>
                            <div className="text-base font-bold text-rose-700">{officer.failed}</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-amber-600">Skipped</div>
                            <div className="text-base font-bold text-amber-700">{officer.skipped}</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-slate-400">Avg Response</div>
                            <div className="text-base font-bold text-slate-900">{officer.avgResponseTime}s</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-slate-400">Reports Today</div>
                            <div className="text-base font-bold text-slate-900">{officer.todayReports}</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-slate-400">Citizens</div>
                            <div className="text-base font-bold text-slate-900">{officer.citizenCount}</div>
                          </div>
                          <div className="p-2.5 bg-white rounded-lg border border-slate-200 text-center">
                            <div className="text-[10px] uppercase font-bold text-slate-400">Attendance</div>
                            <div className="text-xs font-bold text-slate-900 capitalize pt-1">{officer.attendanceStatus}</div>
                          </div>
                        </div>
                          ...
                          <div className="bg-white rounded-xl border border-slate-200 overflow-hidden">
                            ...
                            <div className="overflow-x-auto">
                              <table className="w-full text-left text-xs text-slate-700">
                                <thead className="bg-slate-50/50 text-[10px] uppercase tracking-wider text-slate-500 border-b border-slate-200">
                                  <tr>
                                ...
                                <tbody className="divide-y divide-slate-100">
                                  {officer.questionHistory.slice(0, 10).map((item, idx) => (
                                    <tr key={idx} className="hover:bg-slate-50/50">
                                      ...
                                      <td className="py-2 px-4 text-center font-mono text-[11px] text-slate-500">
                                        {item.responseTime}s
                                      </td>
